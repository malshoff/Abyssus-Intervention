        ...
        if (ImGui::Begin("Cheat Menu", &show_menu, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings))
        {
            ...
            ImGui::BeginChild("##sidebar", ImVec2(200, 0), true);
            {
                ...
                auto SidebarButton = [&](MenuTab tab, const char *icon, const char *label)
                {
                    ...
                    ImU32 col = 0;
                    if (selected)
                        col = ImGui::GetColorU32(bgSelected);
                    else if (ImGui::IsMouseHoveringRect(p0, p1))
                        col = ImGui::GetColorU32(bgHover);
                    if (col)
                        dl->AddRectFilled(p0, p1, col, style.FrameRounding);
                ...
                SidebarButton(MenuTab::WEAPONS, kTabIconWeapons, "Weapon Mods");
                SidebarButton(MenuTab::GAMEPLAY, kTabIconGameplay, "Gameplay Mods");
                SidebarButton(MenuTab::UNLOCKS, kTabIconUnlocks, "Unlocks");
                SidebarButton(MenuTab::OPTIONS, kTabIconOptions, "Visuals");
