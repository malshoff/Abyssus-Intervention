...
namespace Cheat
{
    namespace Services
    {
        ...
        static void Draw2DBoxFromCapsule(SDK::AActor *actor, SDK::APlayerController *controller)
        {
            ...
            if (!Math::WorldToScreen(headWorld, &headScreen, controller))
                return;
            if (!Math::WorldToScreen(feetWorld, &feetScreen, controller))
                return;
            ...
            if (!Math::WorldToScreen(midWorld, &midScreen, controller))
                return;
            if (!Math::WorldToScreen(midRightWorld, &midRightScreen, controller))
                return;
        ...
        void ESPService::Update()
        {
            ...
            for (auto *actor : targets)
            {
                ...
                if (!Cheat::Services::TargetingService::IsValidTarget(actor))
                    continue;
