...
namespace Cheat
{
    namespace Services
    {
        ...
        void PlayerEffectsService::Shutdown()
        {
            // Try to restore any modified movement values on the current pawn
            auto *world = GameServices::GetWorld();
            if (!world)
                return;
            auto *controller = GameServices::GetPlayerController();
            if (!controller)
                return;
            auto *pawn = static_cast<SDK::ARPlayerPawn *>(controller->K2_GetPawn());
            if (!pawn)
                return;
            auto *moveComp = pawn->GetRPawnMovementComponent();
            if (!moveComp)
                return;
